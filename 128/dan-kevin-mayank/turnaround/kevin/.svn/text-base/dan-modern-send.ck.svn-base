// fast high notes +3
// fast low notes (0)
// slow chords with a little ditty (+2)
// fast chords with a tapered low notes at end (0)
// fast high notes +3, with it everywhere...then repeat


// Create OSC Sender
6 => int numStations;   // # of SLOrk stations (up to 6)
7777 => int port;   // same listening port # for all stations
...
while( true )
{
    ...
    while( hi.recv( msg ) )
    {
        ...
        if( msg.isButtonDown() )
        {
            ...
            (curInst+1) % 6 => curInst;
              if (msg.key == 29) {
                  spork ~ play_single_note(root + notescale*12, vel, curInst);
            } if (msg.key == 27) {
                  spork ~ play_single_note(root+2 + notescale*12, vel, curInst);
            } if (msg.key == 6) {
                  spork ~ play_single_note(root+3 + notescale*12, vel, curInst);
            } if (msg.key == 25) {
                  spork ~ play_single_note(root+5 + notescale*12, vel, curInst);
            } if (msg.key == 5) {
                  spork ~ play_single_note(root+7 + notescale*12, vel, curInst);
            } if (msg.key == 4) {
                  spork ~ play_single_note(root+7 + notescale*12, vel, curInst);
            } if (msg.key == 22) {
                  spork ~ play_single_note(root+8 + notescale*12, vel, curInst);
            } if (msg.key == 7) {
                  spork ~ play_single_note(root+11 + notescale*12, vel, curInst);
            } if (msg.key == 9) {
                  spork ~ play_single_note(root+12 + notescale*12, vel, curInst);
            } if (msg.key == 20) {
                  spork ~ play_single_note(root+12 + notescale*12, vel, curInst);
            } if (msg.key == 26) {
                  spork ~ play_single_note(root+14 + notescale*12, vel, curInst);
            } if (msg.key == 8) {
                  spork ~ play_single_note(root+15 + notescale*12, vel, curInst);
            } if (msg.key == 21) {
                  spork ~ play_single_note(root+17 + notescale*12, vel, curInst);
            } if (msg.key == 23) {
                  spork ~ play_single_note(root+19 + notescale*12, vel, curInst);
            }
            ...
            // Send surround
            if ( toggleSurround || msg.key == 224 || msg.key == 227 || msg.key == 226 || msg.key == 230 || msg.key == 231) {
                ...
                {
                    (curInst % numStations) => int j;
                    if (i == j || msg.key == 224 || msg.key == 227 || msg.key == 226 || msg.key == 230 || msg.key == 231) {
                        // start the message...
                        (curInst % numStations) => int i;
                        xmit[i].startMsg( "key/delay/vel, i, f, f" );
                        msg.key => xmit[i].addInt;
                        
                        Std.rand2f(0.0, 0.5) => delay[i] => xmit[i].addFloat;
                        vel - 0.05 => xmit[i].addFloat;
