...
void getPitch(float freq, string &pitch);
...
void displayFunc( )
{
    ...
    glPushMatrix();
		...
		vector<float> fftNew(ZPF*g_buffSize/2),audioNew(g_buffSize);
		...
		static deque< vector<float> > fftHistory;
		static deque< vector<float> > audioHistory;
		...
		{
			fftNew[i] = cmp_abs(fft[i]);
			if ( fftNew[i] > max_energy )
					{
						max_energy = fftNew[i];
		...
		getPitch(primary_frequency,pitch);
		
		draw_string(0,0.5,0,pitch,1);
		//delete pitch;
			for(size_t i = 0; i < g_buffSize; ++i)
			{
				audioNew[i]=g_audioBuff[i];
			}
		g_mutex.unlock();

		fftHistory.push_front(fftNew);
		audioHistory.push_front(audioNew);
		
		if (fftHistory.size() > g_fft_history) fftHistory.pop_back();
		if (audioHistory.size() > g_audio_history) audioHistory.pop_back();
	...
	{int k=0;
				static float z=0.1;
				for(deque< vector<float> >::iterator j = fftHistory.begin(); j != fftHistory.end() ; j++)	
				{

	                glBegin(GL_LINE_STRIP);
					for(size_t i = 0; i < ZPF*g_buffSize/2; ++i)
                	{	
						{	glColor4f(0.0,100*(*j)[i]+0.2,100*(*j)[i]+0.2,100*(*j)[i]+0.35);
							theta = 2*((-PI/2)+ k*PI/(g_fft_history));
							amplitude=g_spectrogram*g_side*(5*2 * ZPF * (*j)[i])*cos(theta/2);//(1-(float)j/g_fft_history);
		...
		{int k=0;
			...
			for (deque< vector<float> >::iterator j = audioHistory.begin(); j != audioHistory.end() ; j++)
			{
				glLineWidth(30.0f*(float)k/g_audio_history);
				glBegin(GL_LINE_STRIP);
				for(size_t i = 0; i < g_buffSize; ++i)
	       		{		
						glColor4f(1,0,cos(PI*i/g_buffSize),(1-(float)k/g_audio_history)*sin(PI*i/g_buffSize));
	              		glVertex3f( i*2/((float)g_buffSize), (*j)[i]/4, 0 );
				}
				k++;
				glEnd();
			}
...
// = Pitch detector =
// ============
void getPitch(float freq,string &return_value)
{	
	if (freq==0) {return_value="z";return;}
