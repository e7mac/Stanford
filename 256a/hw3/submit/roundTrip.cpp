...
int g_speed_inverse=20; //waterfall speed inverse
int g_side = 1;
...
void keyboardFunc( unsigned char key, int x, int y )
{
    ...
    case 'T':
		...
		cerr<<"Switching between line and cylinder view"<<endl;
	break;
    case 'z':
    case 'Z':
       if( !g_fullscreen )
	...
	case '+':
	case '=':
		if (g_speed_inverse>1) g_speed_inverse--;
	case '-':
    case '_':
		g_speed_inverse++;
		break;
...
void displayFunc( )
{
    ...
    glPushMatrix();
		...
		static const int fftHistoryNumber=100;
		static const int audioHistoryLines=100;
		//buffer array to hold previous buffer data
		static vector< vector<float> > fftHistory(fftHistoryNumber,vector<float>(ZPF*g_buffSize/2));
		static vector< vector<float> > audioHistory(audioHistoryLines,vector<float>(g_buffSize));
		
		//animationTick to define speed of cylinder roll
		...
		animationTick++;
		animationTick = (animationTick)%(2*g_speed_inverse);
		...
		g_mutex.unlock();
		//if (animationTick==0)
		{for (int j=fftHistoryNumber-1;j>0;j--)
			{
				fftHistory[j]= fftHistory[j-1];
			}
		for (int j=audioHistoryLines-1;j>0;j--)
			{
	...
	if( g_fftBuff)
        {
				static float z=0.1;
				for(size_t j = 0; j < fftHistoryNumber; j++)	
               	{
                	...
                	{	
						{	glColor4f(0.0,100*fftHistory[j][i]+0.2,100*fftHistory[j][i]+0.2,100*fftHistory[j][i]+0.35);
							theta = 2*((-PI/2)+j*PI/(fftHistoryNumber));
							amplitude=g_spectrogram*g_side*(5*2 * ZPF * fftHistory[j][i])*cos(theta/2);//(1-(float)j/fftHistoryNumber);
		...
		{  g_mutex.lock();
			...
			for (int j=0;j < audioHistoryLines;j++)
			{
				glLineWidth(30.0f*(float)j/audioHistoryLines);
				glBegin(GL_LINE_STRIP);
				for(size_t i = 0; i < g_buffSize; ++i)
	       		{	
						
						glColor4f(1,0,cos(PI*i/g_buffSize),(1-(float)j/audioHistoryLines)*sin(PI*i/g_buffSize));
	              		glVertex3f( i*2/((float)g_buffSize), audioHistory[j][i]/4, 0 );
	       				//cout<<(1-(float)j/audioHistoryLines)<<endl
				}
				glEnd();
			}
			g_mutex.unlock();
            ...
            RtAudioStreamStatus status, void * userData )
{
    ...
    for(size_t i = 0; i < bufferSize; ++i)
    {
        ...
        g_fftBuff[i] = ::sin( 2 * M_PI * 440.0f * i / MY_SRATE );
        //g_fftBuff[i] = in[i];
    }
