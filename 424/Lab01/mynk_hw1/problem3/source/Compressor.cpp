...
: AudioEffectX (audioMaster, 1, kNumParams)	// 1 program, 1 parameter only
{
    ...
    rmp_detector.setTauRelease( release_time, the_sample_rate);    
...
void Compressor::setParameter (VstInt32 index, float value)
{
	switch (index)
	{
        ...
        case kParamDetectorExponent:
            DetectorExponentKnob=value;
            detector_exponent = SmartKnob::knob2value(DetectorExponentKnob, DetectorExponentLimits, DetectorExponentTaper);
            rmp_detector.setExponent(detector_exponent);
            break;////////////////////////////////////////////////////////////////////////////
...
void Compressor::getParameterName (VstInt32 index, char* label)
{
	switch (index)
	{
        ...
        case kParamOutputGain:
            ...
            break;
        case kParamDetectorExponent:
            vst_strncpy(label, "Det Exp ",kVstMaxParamStrLen);
            break;
        default :
            *label = '\0';
            break;
...
void Compressor::getParameterDisplay (VstInt32 index, char* text)
{
	switch (index)
	{
        ...
        case kParamRatio:
            ...
            break;
        case kParamDetectorExponent:
            float2string(detector_exponent, text, kVstMaxParamStrLen);
            break;
        default :
            *text = '\0';
            break;
...
void Compressor::processReplacing (float** inputs, float** outputs, VstInt32 sampleFrames)
{
    ...
    float level_estimate, log_level, level_estimate_r,level_estimate_l;
	...
	for (int i = 0; i < sampleFrames; i++)
	{
		...
		///////////  RMS DETECTOR  //////////////////////////
        float input = max(fabs(inp0), fabs(inp1));
        input = pow(input,rmp_detector.p);
        rmp_detector.process(input, level_estimate);
        level_estimate = pow(level_estimate,1/rmp_detector.p);
        ////////////////////////////////////////////////////////////////////////////
