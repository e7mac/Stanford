...
struct PeakDetector {
	...
	PeakDetector() {
		...
		a1_r = 0; // release coeffs
        b0_r = 1; 
		a1_a = 0; // attack coeffs
		b0_a = 1;
		reset();
	}
...
struct RMpDetector {
	...
	void process (float input, float& output) {
        levelEstimate = a1*levelEstimate + b0*input;
        output = levelEstimate;
	}
};
...
// Detector exponent limits; tapper, exponent
const static float DetectorExponentLimits[2] = {2, 20};
const static float DetectorExponentTaper = -1.0;
