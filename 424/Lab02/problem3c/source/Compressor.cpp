...
: AudioEffectX (audioMaster, 1, kNumParams)	// 1 program, 1 parameter only
{
    ...
    peak_detector_slow.setTauAttack( attack_time, the_sample_rate);
    peak_detector_slow.setTauRelease( 4, the_sample_rate);
...
void Compressor::setParameter (VstInt32 index, float value)
{
	switch (index)
	{
        ...
        case kParamAttack :
            ...
            attack_time = SmartKnob::knob2value(AttackKnob, ARateLimits, ARateTaper);
            peak_detector.setTauAttack( attack_time, the_sample_rate );
//            peak_detector_slow.setTauAttack( 100*attack_time, the_sample_rate );            
            break;
        case kParamRelease:
            ReleaseKnob = value;
            release_time = SmartKnob::knob2value(ReleaseKnob, RRateLimits, RRateTaper);
            peak_detector.setTauRelease( release_time, the_sample_rate );
//            peak_detector_slow.setTauRelease( 100*release_time, the_sample_rate );            
            break;
...
void Compressor::processReplacing (float** inputs, float** outputs, VstInt32 sampleFrames)
{
    ...
    float level_estimate, level_estimate_slow, log_level;
	...
	for (int i = 0; i < sampleFrames; i++)
	{
        ...
        peak_detector_slow.process(std::max(fabs(inp0), fabs(inp1)), level_estimate_slow);
        
        static bool setToOne = false;
        if (level_estimate/level_estimate_slow<0.3 && release_time<1)
        {
            if (!setToOne)
            {
                peak_detector.setTauRelease(1, the_sample_rate);
                setToOne = true;
            }
        }
        else
