...
protected:
	float	b0_r, a1_r, b0_a, a1_a, b0_slow, levelEstimate;
...
public:
    ...
    void setSlowRelease(float fs)
    {
        b0_slow = 1 - exp( -1.0 / ( 1 * fs ) );
    }
	...
	void process (float input, float& output) {        
		if ( fabs( input ) > levelEstimate )
            ...
            levelEstimate += b0_slow * ( fabs( input ) - levelEstimate );
...
protected:
    ...
    PeakDetector peak_detector_slow;
};
...
const static float RatioLimits[2] = {1, 1000.0};
