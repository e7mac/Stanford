...
: AudioEffectX (audioMaster, kNumProgs, kNumParams)	// 1 program, 1 parameter only
{
    ...
    infcSlewer.setTau(.02, fs); // set the mod signal slewer
	ingSlewer.setTau(.02, fs); // set the mod signal slewer	
	inqSlewer.setTau(.02, fs); // set the mod signal slewer

    outfcSlewer.setTau(.02, fs); // set the mod signal slewer
	outgSlewer.setTau(.02, fs); // set the mod signal slewer	
	outqSlewer.setTau(.02, fs); // set the mod signal slewer
    ...
    designHighPass(DcCoefs, 5, 8);
    DcFilter.setCoefs(DcCoefs);
...
void Distortion::getParameterName(VstInt32 index, char *label)
{
	...
	switch (index)
	{
        ...
        case kParamGainIn:
            // input filter gain, dB
            vst_strncpy(label, "InFilGain", kVstMaxParamStrLen);
            break;
            
        case kParamFcIn:
            // input filter center frequency, Hz
            vst_strncpy(label, "InFilFc", kVstMaxParamStrLen);
            break;
            
        case kParamQIn:
            // input filter resonance, ratio
            vst_strncpy(label, "InFilQ", kVstMaxParamStrLen);
            break;
            
        case kParamGainOut:
            // output filter gain, dB
            vst_strncpy(label, "OutFilGain", kVstMaxParamStrLen);
            break;
            
        case kParamFcOut:
            // output filter center frequency, Hz
            vst_strncpy(label, "OutFilFc ", kVstMaxParamStrLen);
            break;
            
        case kParamQOut:
            // output filter resonance, ratio
            vst_strncpy(label, "OutFilQ", kVstMaxParamStrLen);
            break;
...
// overwrite output
{
	...
	double isignal, fsignal, osignal, usignal, dsignal, dcosignal;
	...
	{        
		...
		for (k = 0; k < kUSRatio; k++) {
            ...
            dsignal = (usignal + dcOff) / (1.0 + fabs(usignal + dcOff));
//            dsignal = fmin(1.0, fmax(-1.0, usignal));
            
            //dc blocker
            DcFilter.process(dsignal, dcosignal);
//            dcosignal = dsignal;
			// apply antialiasing filter
			for (j = 0; j < kAAOrder; j++) {
				AAFilter[j].process(dcosignal,dcosignal);
			}
            dcosignal*=SystemGain;
		}
        
        
		// apply output gain, output filter
		OutFilter.process(level*dcosignal, osignal);
...
//------------------------------------------------------------------------------
void Distortion::bilinearTranform(double acoefs[], double dcoefs[])
{
...
//------------------------------------------------------------------------------
void Distortion::bilinearTranformUpsampled(double acoefs[], double dcoefs[],float upsample)
{
	double b0, b1, b2, a0, a1, a2;		//storage for continuous-time filter coefs
	double bz0, bz1, bz2, az0, az1, az2;	// coefs for discrete-time filter.
	...
	float newFs = fs * upsample;
	// TODO: apply bilinear transform
	///////////////START//////////////////
    double az0_old = (4*newFs*newFs*a2+2*newFs*a1+a0);
	bz0 = (4*newFs*newFs*b2-2*newFs*b1+b0)/ az0_old; 
    bz1 = (2*b0-8*newFs*newFs*b2)/az0_old; 
    bz2 = (4*newFs*newFs*b2-2*newFs*b1+b0)/az0_old; 
    az0 = 1.0; 
    az1 = (2*a0-8*newFs*newFs*a2)/az0_old;
    az2 = (4*newFs*newFs*a2-2*newFs*a1+a0)/az0_old;     
...
// design parametric filter based on input center frequency, gain, Q and sampling rate
{
    ...
    a0 = 1.0; a1 = 0.0; a2 = 0.0;
 	///////////////START//////////////////
    b0 = 1.0; b1 = dB2mag(gain)/(qval*2*pi*center); b2 = 1/(4*pi*pi*center*center); 
...
void Distortion::designHighPass(double* dcoefs, double center, float upsample)
// design parametric filter based on input center frequency, gain, Q and sampling rate
{
	double b0, b1, b2, a0, a1, a2;		//storage for continuous-time filter coefs
	double acoefs[6];
    ...
    a0 = 2*pi*center*2*pi*center; a1 = 2*2*pi*center; a2 = 1.0;   
	...
	// inputs the 6 analog coeffs, output the 5 digital coeffs
	bilinearTranformUpsampled(acoefs, dcoefs, 8);
